...
async def handler(websocket, path):
    try:
        async for message in websocket:
            ...
            # stop — wyślij SIGINT (jak Ctrl+C) do wszystkich uruchomionych procesów
            if parts[0] == "stop":
                stop_all_commands()
                continue
            ...
            # point execute
            if parts[0] == "point" and len(parts) >= 2 and parts[1] == "execute":
                if not point_list:
                    continue
            ...
            cmd_key = parts[0]  # ignorujemy tekst po spacji dla dotychczasowych komend
